...
# Use the virtual environment's libs
try:
    import websockets
except ImportError:
...
async def main():
    ...
    with wave.open("/tmp/verify_host.wav", "rb") as f:
    ...
    # Connect to WebSocket in Immediate Mode (chunk_ms=0)
    uri = "ws://localhost:8000/api/stream?language=hi-IN&denoise=true&vad=true&chunk_ms=0"
                ...
                async for msg in ws:
                    ...
                    if data.get("event") == "transcript":
                        ...
                        if text:
                            ...
                            # span_end * 16000 samples / (chunk_size / 2)
                            ...
                            if idx in send_times:
                                ...
                                print(f"[{span_start:.2f}s - {span_end:.2f}s] Latency: {latency*1000:.1f}ms | Final: {is_final} | Text: {text}")
